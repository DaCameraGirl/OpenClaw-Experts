...
const STARTERS = {
  gitRecovery: {
    ...
    prompt:
      "I accidentally force-pushed and I think I lost recent work in this repo. Use the installed Git or shell skill to inspect the repository history, reflog, branches, and working tree. Compare and score candidate commits, figure out which commits are safe to recover, create a recovery branch without overwriting the current branch, and write ./artifacts/git_recovery_report.md. Keep durable repository facts in MEMORY.md so you do not repeat the same investigation. Do not run destructive reset or checkout commands against the current branch. The report must include recovered_commit, evidence_from_log, evidence_from_reflog, files_changed, recovery_branch, and remaining_risks.",
    outcome:
  ...
  typescriptBug: {
    ...
    prompt:
      "This TypeScript project has a conditional type bug. Use the installed shell or TypeScript skill to inspect the source and compiler output, compare candidate fixes, find the smallest safe fix, and run the typecheck. Keep durable project facts in MEMORY.md. Do not weaken strictness with any, unknown-as-any, ts-ignore, or broad casts. Write ./artifacts/typescript_fix_report.md with failing_type, root_cause, files_changed, why_the_fix_is_narrow, typecheck_result, and remaining_risks.",
    outcome:
  ...
  reactRace: {
    ...
    prompt:
      "This React app has an async UI bug that looks like a stale closure or race condition. Use the installed shell or React skill to inspect the component code and relevant tests, compare candidate fixes, identify the actual state-flow problem, apply the smallest safe patch, and run the relevant tests or build. Keep durable project facts in MEMORY.md. Do not hide the bug with arbitrary timeouts or by disabling UI updates. Write ./artifacts/react_race_fix_report.md with component, race_trigger, root_cause, files_changed, test_or_build_result, and remaining_risks.",
    outcome:
...
function runQualityGates(draft) {
  ...
  const gates = [
    ...
    gate("skills", "Installed skill requirement is forced by prompt and field", draft.requiredSkill.trim() && (prompt.includes("skill") || prompt.includes(draft.requiredSkill.toLowerCase())), "17.5"),
